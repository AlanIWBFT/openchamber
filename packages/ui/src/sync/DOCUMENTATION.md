...
`sessionStatusReady` records whether the directory has received a successful live status snapshot. Once ready, an omitted session is authoritatively idle. Before then, incomplete message metadata may provide only a bounded startup fallback ending at the directory's `session_status_fallback_until`; navigation and remounting must not extend it. Fetch failure preserves the unresolved state.

`directory-recovery-snapshots.ts` owns status request generations and in-flight event reconciliation for bootstrap and reconnect/watchdog reads. Callers claim a generation only after a successful read and their runtime/store checks. A newer successful snapshot supersedes older responses; a newer failed request leaves an earlier usable response eligible. The shared reader preserves intervening SSE and optimistic transitions, including repeated events that cause no store publication, and keeps nullable read failure distinct from empty success. Changed store entries take precedence over recorded events because a new optimistic turn may follow an idle event while the same read is pending. Recorded events still protect entries whose store reference did not change.

Retry snapshots and compatibility events retain recovery metadata such as `action` and `resolution`. The SDK boundary validates those fields before granting snapshot authority. Real reconnect and transport-switch recovery refreshes the full directory snapshot; existing candidates scope follow-up message materialization. Once status is ready, incomplete historical messages no longer protect an idle session from cache eviction.
...
11. Persisted part arrays are ordered only by part/event `seq`. Part IDs are identity keys and have the same rollover limitation; identity lookup/removal must not require a part array to be lexically ID-sorted.
...
13. Explicit user Stop actions call `session.stop`; transport failures, reconnects, refreshes, and session navigation do not.
