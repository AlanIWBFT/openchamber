...
describe("optimisticSend target directory", () => {
  ...
  test("loads a missing revert boundary before committing a new branch", async () => {
    ...
    const revertedTail = { id: "msg_3", role: "assistant", sessionID: "session-reverted", time: { created: 3 } } as Message
    const targetStore = createStore({}, {
  ...
  test("does not send when a missing revert boundary cannot be loaded", async () => {
    const cachedTail = { id: "msg_3", role: "assistant", sessionID: "session-reverted", time: { created: 3 } } as Message
    const targetStore = createStore({}, {
...
describe("revertToMessage passes session directory", () => {
  ...
  test("reverts recursive descendants at their first user message on or after the parent cutoff", async () => {
    ...
    sessionMessagesResult = { data: [{ info: { ...rootMessage, seq: 1 }, parts: [] }] }
    const sessions = [
