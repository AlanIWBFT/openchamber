...
const ChatInputComponent: React.FC<ChatInputProps> = ({
    ...
    const handleAbort = React.useCallback(() => {
        ...
        void stopCurrentOperation(targetSessionId)
            .then(() => {
                if (useSessionUIStore.getState().currentSessionId === currentSessionId) startAbortIndicator();
            })
            .catch((error) => {
                console.error('[chat-input] stop failed', error);
                toast.error(error instanceof Error ? error.message : 'Failed to stop session');
            });
    }, [btwSessionId, clearAbortPrompt, currentSessionId, isBtwActive, startAbortIndicator, stopCurrentOperation]);
