...
describe('turnProjectionCache', () => {
  test('keeps the cache key stable for unchanged message and part references', () => {
    ...
    const first = buildProjectionCacheKey('session_1', messages, false, false, true, 'merge');
    const second = buildProjectionCacheKey('session_1', messages, false, false, true, 'merge');
  ...
  test('changes the cache key when streaming replaces a part with the same id and count', () => {
    ...
    const beforeKey = buildProjectionCacheKey('session_1', before, false, false, true, 'merge');
    const afterKey = buildProjectionCacheKey('session_1', after, false, false, true, 'merge');
