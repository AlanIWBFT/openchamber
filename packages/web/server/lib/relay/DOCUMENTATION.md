...
- `service.js` exposes `shutdown()` for process teardown. Unlike the normal
  user-facing `stop()`, shutdown permanently closes admission so an in-flight
  reconcile cannot recreate the relay host while the process is exiting.
