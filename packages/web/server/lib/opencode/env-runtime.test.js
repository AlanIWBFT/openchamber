...
describe('OpenCode env runtime', () => {
  ...
  it('requires a sibling marker before enabling the private SQLite finalizer', () => {
    const dir = createTempDir('openchamber-opencode-finalizer-');
    const binary = path.join(dir, process.platform === 'win32' ? 'opencode.exe' : 'opencode');
    fs.writeFileSync(binary, '#!/bin/sh\nexit 0\n');
    if (process.platform !== 'win32') fs.chmodSync(binary, 0o755);
    const { runtime } = createRuntime({});

    expect(runtime.supportsOpenCodeSqliteFinalizer(binary)).toBe(false);

    fs.writeFileSync(path.join(dir, 'openchamber-sqlite-finalizer.capability'), '1\n');
    expect(runtime.supportsOpenCodeSqliteFinalizer(binary)).toBe(true);
  });
