...
- `packages/web/server/lib/git/`: Git module directory containing all Git-related functionality.
  ...
  - `git-read-worker-client.js` / `git-read-worker.js`: Fixed pool of four persistent Windows Worker Threads for latency-sensitive Git status, PR-context, and per-file diff reads.
  - `process-broker.js`: Client and lifecycle adapter for the single persistent Windows process broker staged with local OpenCode builds. Main-thread Git and all four Git workers share the broker, while each runtime thread keeps its own named-pipe connection.
...
- `getTrackingBranch(directory)`: Upstream of the checked-out branch as `remote/branch` (the same value as `status.tracking`, including an upstream whose remote ref is gone), or `null` when HEAD is detached or unborn or no upstream is configured. Reads refs and config only. Callers that need just the tracking name, such as PR creation, use this instead of `getStatus`; PR status polling uses the worker-backed `getPrGitContext` below.
- Status routes keep their 30-second hard deadline. Simple-git also bounds output stalls, and bounded listings terminate their owned process tree before releasing the read. Broker-backed Windows commands retain Job Object ownership through close acknowledgement.
- Buffered broker exec also honors `timeout`; expiry cancels its Job Object and rejects only after close. The worker-backed direct status path applies the unsupported-root check to its existing `rev-parse` result, without an extra repository probe.
...
- Local Windows desktop builds route Git through one persistent GUI-subsystem process broker shared by the server and all Git workers. The broker is a self-contained .NET 10 NativeAOT executable and does not require a separately installed .NET runtime. Each Git command starts with `DETACHED_PROCESS`, explicit inherited pipe handles, and atomic Job Object membership; cancellation terminates the complete command tree. Stdout and stderr use independent bounded credit windows, and the broker schedules ready output round-robin by command so an unread or noisy stream cannot pause unrelated commands on the same connection. Broker-backed simple-git commands wait for `close` rather than treating `exit` as completion, so output cleanup and a later Broker failure remain authoritative. Standalone web runtimes and upstream OpenCode bundles without the broker sidecar retain the normal `child_process` path.
...
- A worker keeps its Node owner alive only while its caller awaits an active request. Completion or cancellation releases that reference. A cancelled lane still waits for the worker acknowledgement before reuse, but it does not prevent process exit during that recovery period.
- Broker `Cancel` frames with an empty payload are no-ops when the command ID is no longer active, including unknown IDs. Completion can overtake cancellation while the client consumes buffered output; a late Cancel must not close the shared connection or terminate unrelated commands. A nonempty Cancel payload remains a protocol error.
- Late stdin writes and closes receive a command-local broken-pipe acknowledgement after spawn failure or completion. The exec-file adapter handles stdin errors and waits for command cleanup; unavailable executables or working directories must not disconnect concurrent commands. Worktree tests wait for bootstrap completion before deleting their fixtures, matching the product's removal ownership.
