...
describe('git worktree topology routes', () => {
  ...
  it.each(['/api/git/status', '/api/git/status/passive'])('observes repository topology through %s without a separate repository probe', async (routePath) => {
    gitLibraries.getStatus
      .mockResolvedValueOnce({ isGitRepository: true, current: 'main' })
      .mockResolvedValueOnce({ isGitRepository: false });
    const { app, getRoute } = createRouteRegistry();
    registerGitRoutes(app);
    const route = getRoute('GET', routePath);

    const repoResponse = createMockResponse();
    await route({ query: { directory: '/repo' } }, repoResponse);
    expect(repoResponse.body).toEqual({ isGitRepository: true, current: 'main' });
...
describe('git file diff worker availability', () => {
  ...
  it('aborts a file diff that exceeds the hard deadline', async () => {
    ...
    await vi.advanceTimersByTimeAsync(0);
    expect(gitLibraries.getFileDiff).toHaveBeenCalledTimes(1);
    await vi.advanceTimersByTimeAsync(30_000);
    await request;
