...
const launchDetachedOpenCodeKiller = (processInfo) => {
  ...
  if (process.platform === 'win32') {
    if (!hasPid) return;
    const child = spawn('taskkill', ['/pid', normalizedPid, '/f', '/t'], {
      detached: true,
      stdio: 'ignore',
      windowsHide: true,
    });
  ...
  const child = spawn('/bin/sh', ['-c', script, 'openchamber-opencode-killer', normalizedPid, normalizedPort], {
    detached: true,
    stdio: 'ignore',
    windowsHide: true,
  });
...
const stopSidecar = async () => {
  ...
  try {
    const result = await serverModule.stopManagedOpenCode({ deadline });
    if (result?.finalized === false) {
      log.info('[electron] managed OpenCode had no database finalization work');
    }
  } catch (error) {
    log.warn('[electron] OpenCode termination or SQLite finalization failed:', error);
    if (isOpenCodeTerminationFailure(error)) {
