...
describe('quit page lifecycle', () => {
  ...
  test('only process termination failures require the detached killer', () => {
    expect(isOpenCodeTerminationFailure({ openChamberShutdownPhase: 'termination' })).toBe(true);
    expect(isOpenCodeTerminationFailure({ openChamberShutdownPhase: 'finalization' })).toBe(false);
    expect(isOpenCodeTerminationFailure(new Error('unknown'))).toBe(false);
  });
